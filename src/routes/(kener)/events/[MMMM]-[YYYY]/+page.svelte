...
<svelte:head>
  ...
  <meta property="og:image:alt" content={`${currentMonth} events for ${data.siteName || "Zirel"}`} />
  <meta name="twitter:image" content={ogImage} />
  <meta name="twitter:image:alt" content={`${currentMonth} events for ${data.siteName || "Zirel"}`} />
</svelte:head>
...
<div class="public-page">
  ...
  {:else if eventsByDay.length === 0}
    <!--
      Empty state — matches the Console's `EmptyStateCard` rhythm: a
      single card with a tight title / description stack, centered,
      zero decorations beyond the card surface itself.
    -->
  ...
  <!-- Month navigation buttons (bottom) -->
